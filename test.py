...
class PatrollingGraphRoutingProblem:
	...
	def step(self, new_positions: np.ndarray):
		...
		for i in range(self.n_agents):
			...
			self.rho_next[i] = self.L.nodes[self.agent_positions[i]]['value']
			self.rho_act[i] = self.L.nodes[self.agent_pos_ant[i]]['value']

# Calcular el valor 'Visited' para cada agente y almacenarlo en self.rewards
		self.rewards['Visited'] = {}  # Crear un diccionario para almacenar los valores 'Visited' por agente
		for i in range(self.n_agents):
			visited_value = self.rho_next[i] - self.rho_act[i]
			self.rewards['Visited'][i] = visited_value
